...
def parse_args():
    ...
    parser.add_argument('--risbench_root', type=str, default='', help='Root directory for RISBench dataset')
    parser.add_argument('--split', type=str, default='test', help='Dataset split to use (train, val, test)')
    parser.add_argument('--resize_size', type=int, default=840, help='Resize size for input images')
    parser.add_argument('--IoU_threshold', type=float, default=0.5, help='IoU threshold for Precision@X calculation. If not specified, Precision@X will not be calculated.')
...
def main(local_rank, world_size, rank):
    ...
    if args.dataset == 'earthreason':
        from open_r1.dataset.EarthReason_datasets import EarthReasonDataset, EarthReasonDataset_test
        ...
        dataset = RefSegRSDataset(data_root=args.refsegrs_root, split=args.split, resize_size=args.resize_size)
    ...
    if rank == 0:
        ...
        if precision_at_x is not None:
            print(f"Precision@{args.IoU_threshold}: {precision_at_x:.4f} ({precision_at_x_count}/{total_predictions})")
